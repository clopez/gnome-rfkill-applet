 ...
 *
 * Ask the special device /dev/rfkill for the status of RF devices and updates
 * the tooltip and status of the applet.
 * If we detect *only one* device radiating we will set the status to
 * RADIATION_EMITTING.
 *
 * We only will set the status to RADIATION_KILLED if ALL RF devices are blocked.
 *
 *
 * If we detect any problem or we are unable to access /dev/rfkill we will set
 * the status to RADIATION_UNKNOW
 *
 **/
static gboolean
rfkill_get_status (RFKillApplet *applet)
{
 ...
 * @applet: RFKill applet instance
 *
 * We flip-flop our status:
 *
 *  We kill the radiation of all devices if we are in status RADIATION_EMITTING
 *  or in status RADIATION_UNKNOW.
 *
 *  We enable devices to radiate only if we are previous in status RADIATION_KILLED.
 *
 * In case of failure we return FALSE and update the tooltip.
 *
 **/
gboolean rfkill_change_status (RFKillApplet *applet)
{
     ...
     */
    event.soft = (applet->status == RADIATION_KILLED)? 0 : 1;
